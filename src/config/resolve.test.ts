...

{
  ...
  assert.equal(cfg.rules.timer.questionSec, 30, 'defaults fill in')
...
}
{
  const parent: GameShowConfigInput = {
    ...
    rules: { timer: { questionSec: 45 }, scoring: { multiplier: 3 } },
  }
  ...
  assert.equal(cfg.rules.scoring.multiplier, 3, 'parent value survives')
  assert.equal(cfg.rules.timer.hostCanPause, true, 'default survives both layers')
}
{
  assert.throws(
    () => resolveConfig(
...

{
  ...
  const round = {
    id: 'r2', title: 'Double',
    style: { kind: 'grid', columns: 5, rows: 5, pointLadder: [], 
  ...
  assert.equal(rr.timer.hostCanPause, true, 'unrelated defaults survive round override')
...

{
  const q: Question = {
    ...
    acceptedAnswers: ['SECRET'], hostNote: 'private', points: 100,
  }
...

{
  ...
  {
    const shown = redactQuestion(q, 'stage')
    const wire = JSON.stringify(shown)

    for (const secret of ['Books', 'accept plurals', 'Lunch']) {
      assert.equal(
        wire.includes(secret), false,
        `stage payload must not contain ${JSON.stringify(secret)} — got ${wire}`,
      )
    }
    assert.equal(shown.prompt, q.prompt, 'the stage still needs the prompt')
    assert.equal(shown.points, 100, 'the stage still needs the point value')
  }
