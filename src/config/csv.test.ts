...
// A survey sheet is one row per ANSWER, grouped by the question above it.
{
  const sheet = [
    'question,answer,points',
    'Name something in a school bag,Books,38',
    'Name something in a school bag,Pencil case,27',
    'Name a school subject,Maths,40',
    'Name a school subject,English,30',
  ].join('\n')

  const { bank, problems } = readPacketText(sheet, 'packets/feud.csv')
  assert.equal(hasErrors(problems), false, 'a survey sheet needs no category column')
  assert.ok(bank)
  assert.equal(bank.categories.length, 1, 'a survey packet is one category of questions')
  assert.equal(bank.categories[0]?.questions.length, 2, 'rows group into questions')

  const first = bank.categories[0]?.questions[0]
  assert.equal(first?.kind, 'survey')
  assert.equal(first?.prompt, 'Name something in a school bag')
  assert.deepEqual(first?.surveyAnswers, [
    { text: 'Books', count: 38 },
    { text: 'Pencil case', count: 27 },
  ], 'answers keep their row order, which is the order the board shows them')
  assert.equal(first?.answer, 'Books', 'the single answer field falls back to the top one')
}

{
  // The two sheet shapes are told apart by required columns that do not overlap.
  const quiz = 'category,prompt,answer\nSpace,Red planet?,Mars'
  const { bank } = readPacketText(quiz, 'quiz.csv')
  assert.equal(bank?.categories[0]?.questions[0]?.kind, 'text', 'a category column still means a quiz sheet')
}

{
  const sheet = 'question,answer,points\nName a subject,Maths,lots'
  const { bank, problems } = readPacketText(sheet, 'feud.csv')
  assert.equal(bank, null)
  assert.match(problems[0]?.message ?? '', /not a number: "lots"/)
  assert.equal(problems[0]?.where, 'feud.csv row 2', 'survey rows report spreadsheet numbers too')
}

{
  const sheet = 'question,answer,points\nName a subject,,40'
  const { problems } = readPacketText(sheet, 'feud.csv')
