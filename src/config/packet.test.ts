...

{
  ...
  assert.match(problems[0]?.message ?? '', /use "text" or "survey"/)
}
...
// Survey questions in JSON: the answers list is the answer.
{
  const { bank, problems } = readPacket({
    packet: 1, title: 'Feud',
    categories: [{
      title: 'Survey',
      questions: [{
        prompt: 'Name something in a school bag',
        answers: [{ text: 'Books', points: 38 }, { text: 'Phone', points: 14 }],
      }],
    }],
  }, 'feud.json')

  assert.equal(hasErrors(problems), false, 'an "answers" list implies kind survey')
  const question = bank?.categories[0]?.questions[0]
  assert.equal(question?.kind, 'survey')
  assert.deepEqual(question?.surveyAnswers, [
    { text: 'Books', count: 38 },
    { text: 'Phone', count: 14 },
  ])
  assert.equal(question?.answer, 'Books', 'no separate "answer" field is needed')
}

{
  const { bank, problems } = readPacket({
    packet: 1, title: 'Feud',
    categories: [{ title: 'S', questions: [{ prompt: 'p', kind: 'survey', answers: [{ text: 'a' }] }] }],
  }, 'feud.json')
  assert.equal(bank, null, 'an answer without points is refused')
  assert.match(problems[0]?.message ?? '', /"points" must be a number/)
}

{
  const { bank, problems } = readPacket({
    packet: 1, title: 'Feud',
    categories: [{ title: 'S', questions: [{ prompt: 'p', kind: 'survey' }] }],
  }, 'feud.json')
  assert.equal(bank, null)
  assert.match(problems[0]?.message ?? '', /needs an "answers" array/)
}
