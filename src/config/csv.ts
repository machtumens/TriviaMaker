...
/**
 * A survey sheet is one row per ANSWER, grouped by the question it belongs to —
 * the same grouping the quiz sheet does with categories. The two shapes are told
 * apart by their required columns, which do not overlap: a quiz sheet must have
 * "category", a survey sheet must have "points" and must not have "category".
 */
const SURVEY_REQUIRED_FIELDS = ['prompt', 'answer', 'points'] as const
...
export function csvToPacket(text: string, source: string, title: string): CsvPacketResult {
  ...
  const columns = header.map(heading => {
    const name = heading.trim().toLowerCase().replace(/[^a-z0-9]/g, '')
    return HEADER_ALIASES[name] ?? name
  })

  const looksLikeSurvey = !columns.includes('category')
    && SURVEY_REQUIRED_FIELDS.every(field => columns.includes(field))
  if (looksLikeSurvey) return surveySheet(rows, columns, source, title, problems)
...
function cellOf(row: readonly string[], columns: readonly string[], field: string): string {
  const at = columns.indexOf(field)
  return at === -1 ? '' : (row[at] ?? '').trim()
}

/**
 * Survey sheet: one row per answer. Rows group into questions in the order the
 * questions first appear, and answers keep their row order, which is the order
 * the board shows them in — put the highest-scoring answer first.
 */
function surveySheet(
  ...
  const order: string[] = []
  const byQuestion = new Map<string, Array<{ text: string; points: number }>>()
  const categoryOf = new Map<string, string>()

  rows.slice(1).forEach((row, index) => {
    const where = `${source} row ${index + FIRST_DATA_ROW}`
    const prompt = cellOf(row, columns, 'prompt')
    const answer = cellOf(row, columns, 'answer')
    const points = cellOf(row, columns, 'points')
    ...
    let bucket = byQuestion.get(prompt)
    if (!bucket) {
      bucket = []
      byQuestion.set(prompt, bucket)
      order.push(prompt)
      const round = cellOf(row, columns, 'category') || cellOf(row, columns, 'round')
      if (round !== '') categoryOf.set(prompt, round)
    }
    bucket.push({ text: answer, points: value })
  })

  if (problems.some(problem => problem.level === 'error')) return { object: null, problems }
  if (order.length === 0) {
    problems.push({ level: 'error', where: source, message: 'no answer rows below the header' })
    return { object: null, problems }
  }

  const declared = rows.slice(1).map(row => cellOf(row, columns, 'title')).find(value => value !== '')

  // A survey packet is one category holding every question: the Feud board plays
  // them one at a time, so there are no columns to group them into.
  return {
    object: {
      packet: 1,
      title: declared ?? title,
      categories: [{
        title: declared ?? title,
        questions: order.map(prompt => ({
          prompt,
          kind: 'survey',
          answers: byQuestion.get(prompt) ?? [],
        })),
      }],
    },
    problems,
