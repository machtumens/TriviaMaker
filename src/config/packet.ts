...
export const QUICK_START_STRIKES = 3
 ...
 *
 * Everything except `prompt` and `answer` is optional: ids and `kind` are
 * filled in here so that the file a human writes stays small. Points are
...
type Reporter = (where: string, message: string) => void

/** `answers: [{ text, points }]` — `count` is accepted too, since that is what the schema calls it. */
function readSurveyAnswers(raw: unknown, at: string, error: Reporter):
  Array<{ text: string; count: number }> | null {
  if (!Array.isArray(raw)) {
    error(at, 'a survey question needs an "answers" array')
    return null
  }

  const answers: Array<{ text: string; count: number }> = []
  raw.forEach((entry, index) => {
    const where = `${at} answer ${index + 1}`
    if (!isRecord(entry)) {
      error(where, 'each answer must be an object with "text" and "points"')
      return
    }
    const answerText = text(entry['text'] ?? entry['answer'])
    if (answerText === null) {
      error(where, '"text" is required')
      return
    }
    const rawPoints = entry['points'] ?? entry['count']
    if (typeof rawPoints !== 'number' || !Number.isFinite(rawPoints)) {
      error(where, `"points" must be a number — got ${JSON.stringify(rawPoints)}`)
      return
    }
    answers.push({ text: answerText, count: rawPoints })
  })
...
export function readPacket(raw: unknown, source: string): PacketReadResult {
  ...
  rawCategories.forEach((rawCategory, categoryIndex) => {
    ...
    rawQuestions.forEach((rawQuestion, questionIndex) => {
      ...
      const kind = text(rawQuestion['kind']) ?? (rawAnswers === undefined ? 'text' : 'survey')
      if (kind !== 'text' && kind !== 'survey') {
        error(at, `kind "${kind}" is not playable in this build — use "text" or "survey"`)
      }

      // A survey question's answer is the list; the single `answer` field is
      // filled from the top one so the host card still has something to show.
      const surveyAnswers = kind === 'survey' ? readSurveyAnswers(rawAnswers, at, error) : null
      if (kind === 'survey' && surveyAnswers !== null && surveyAnswers.length === 0) {
        error(at, 'a survey question needs at least one answer in "answers"')
      }
      ...
      const answer = text(rawQuestion['answer']) ?? surveyAnswers?.[0]?.text ?? null
      if (answer === null && kind === 'text') error(at, '"answer" is required')
      ...
      const question: Question = { id: questionId, kind: kind as Question['kind'], prompt, answer }
      if (surveyAnswers !== null) question.surveyAnswers = surveyAnswers
    ...
    categories.push({ id: categoryId, title: categoryTitle, questions })
  })
  ...
  if (hasErrors(problems) || packetId === null || title === null) return { bank: null, problems }
 ...
 * only the board: the format stays, the content changes.
 */
/** Survey content only plays on the Feud board, so a survey packet builds one. */
function isSurveyBank(bank: QuestionBank): boolean {
  return bank.categories.some(category =>
    category.questions.some(question => question.kind === 'survey'))
}

function surveyRound(bank: QuestionBank): Round {
  const slots = Math.max(
    ...bank.categories.flatMap(category =>
      category.questions.map(question => question.surveyAnswers?.length ?? 0)),
    1,
  )
  ...
  return {
    ...
    style: {
      kind: 'list',
      slots,
      strikesAllowed: QUICK_START_STRIKES,
      showCounts: true,
    },
  }
}
...
export function quickStartShow(bank: QuestionBank, base?: GameShowConfigInput): QuickStartResult {
  ...
  if (isSurveyBank(bank)) {
    const round = surveyRound(bank)
    return {
      input: base
        ? { ...base, program: { ...base.program, rounds: [round] } }
        : { ...quickStartFrame(bank), program: { carryScores: true, rounds: [round] } },
      problems,
    }
  }
  ...
  return {
    input: {
      ...quickStartFrame(bank),
      program: { carryScores: true, rounds: [round] },
    },
    problems,
  }
}

/** Everything a packet-only show needs apart from its rounds. */
function quickStartFrame(bank: QuestionBank): GameShowConfigInput {
  return {
    meta: { id: bank.id, title: bank.title },
    teams: { teams: QUICK_START_TEAMS },
    rules: {
      timer: { questionSec: 30, hostCanPause: true },
      wrongAnswer: { penalty: 0 },
      scoring: { engine: 'flat', multiplier: 1 },
    },
    runtime: { transport: { driver: 'local' } },
  }
}
