  ...
  switch (type) {
    ...
    case 'advanceRound': {
      const eliminateTeamId = typeof fields['eliminateTeamId'] === 'string'
        ...
        : undefined
      const raw = fields['advanceTeamIds']
      const advanceTeamIds = Array.isArray(raw)
        ? raw.filter((id): id is string => typeof id === 'string')
        : undefined
      return advanceToNextRound(state, { eliminateTeamId, advanceTeamIds })
    }
    ...
    default: {
      const round = currentRound(state.config, state.roundIndex)
      const style = resolve<StylePlugin>('style', styleKeyFor(round))
      const handled = style.onCommand?.(state, type, fields)
