...
const state: SessionState = {
  ...
  teams: [{ id: 'a', name: 'A', color: '#f00', score: 0, eliminated: false }],
...
{
  assert.equal(calls.length, 2, 'exactly two broadcasts, one per channel')
  assert.deepEqual(
    [...calls.map(c => c.channel)].sort(),
    ['host', 'stage'],
    'the host and the stage each received a payload',
  )
}
...
{
  const name = 'stage'
  const board = boardOf(stagePayload)
...
const REDACTED_FIELDS = ['answer', 'acceptedAnswers', 'hostNote'] as const

{
  const name = 'stage'
  const serialised = JSON.stringify(stagePayload)
  ...
  const question = stagePayload['currentQuestion'] as Record<string, unknown> | null
  ...
  const config = stagePayload['config'] as GameShowConfig
...

{
  ...
  assert.equal(styleCalls.length, 2, 'a populated styleState does not disturb the broadcast')
