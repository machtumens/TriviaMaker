 ...
 * this window, so a packet is read, validated and played client-side.
 */
...
export function mountPacketPicker(): void {
  ...
  const readFile = (picked: File): void => {
    if (!isPacketPath(picked.name)) {
      say(`${picked.name} is not a packet — expected a .json or .csv file`, true)
      return
    }
    picked.text()
      .then(text => { load(picked.name, text, true) })
      .catch((error: unknown) => {
  ...
  picker.addEventListener('change', () => {
    if (picker.value === '') {
      // The page already rebuilds the authored show from the preset on load.
      location.reload()
      return
    }
